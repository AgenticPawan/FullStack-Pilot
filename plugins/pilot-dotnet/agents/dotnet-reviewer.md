...
| dotnet-security-headers | SECH-* | HSTS, X-Content-Type-Options, clickjacking protection, anti-forgery/CSRF on cookie auth, safe JSON deserialization, mass-assignment/over-posting via entity-bound requests |
...
**Category AR — Security headers & request-binding safety**
- [ ] No `UseHsts()` outside Development (SECH-001)?
- [ ] No `X-Content-Type-Options: nosniff` header (SECH-002)?
- [ ] HTML-rendering endpoint with no `X-Frame-Options`/CSP `frame-ancestors` clickjacking protection (SECH-003)?
- [ ] Cookie-authenticated state-changing endpoint with no anti-forgery token validation (SECH-004)?
- [ ] `TypeNameHandling.Auto`/`All` or an unbounded `$type` discriminator resolving untrusted input (SECH-005)?
- [ ] Request parameter bound directly to an EF entity instead of a dedicated request DTO (SECH-006)?
