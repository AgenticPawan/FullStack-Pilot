...
> 174 governance skills and 28 agents for [Claude Code](https://claude.ai/code) — Angular · .NET · SQL Server · Azure
...
FullStack Pilot ships those rules as 174 versioned, stack-specific skills and 28 agents. Every new project starts from a consistent governance baseline rather than a blank slate.
...
Three agents are optimised for specific enterprise roles rather than stack layers. Each has a different permission scope and output format — a shared "do everything" agent produces mediocre results for all three.

| Agent | Persona | What it produces |
|---|---|---|
| `@fsp-feature-builder` | Fullstack Developer | Scaffolds a complete vertical slice (Angular + .NET + EF Core + SQL + Azure) in dependency order, delegates to stack implementors, runs a cross-stack seam check, and verifies the build is clean before reporting done |
| `@fsp-incident-responder` | Production Support Engineer | Receives error artifacts (stack trace, SQL timeout, Azure alert), correlates them into a request-chain timeline, produces a ranked root-cause list with confidence levels, a proposed fix branch, a rollback plan, and a shift-handoff note |
| `@fsp-tpo-intake` | Technical Product Owner | Asks four mandatory clarifying questions before writing anything, then produces an 8-section developer-ready spec at `.claude/specs/<feature>.md` and validates a PR against the spec when given a diff |

**Permission boundaries:**
- `@fsp-feature-builder` — writes to all layers; never claims done if the build fails
- `@fsp-incident-responder` — writes `.claude/pilot/` and one local fix branch **only**; hardcoded `BOUNDARY VIOLATION` protocol stops it on any `kubectl apply`, `az deploy`, `docker run`, or `git push`
- `@fsp-tpo-intake` — writes `.claude/specs/` only; never touches code files

## Plugins

| Plugin | Skills | Agents | Highlights |
|---|---|---|---|
| `pilot-core` | 44 | 13 | `/fsp-init`→`/fsp-build` pipeline skills, `/fsp-checkpoint` · `/fsp-verify` · `/fsp-health`, delivery-team agents, `@fsp-upgrade-planner` · `@fsp-threat-modeler` · `@fsp-debugger`, persona agents (`@fsp-feature-builder` · `@fsp-incident-responder` · `@fsp-tpo-intake`), cross-stack seam review, incident correlation, spec validation, DORA metrics, MCP discovery, CI secret scanning, REST API standards, SLO load testing, git governance, incident-response runbook, license compliance, supply-chain policy |
...
| `pilot-sql` | 12 | 4 | Migration safety, injection defense, schema design, temporal tables/CDC, data retention & GDPR purge, HADR failover (Always On AGs, Azure SQL failover groups), data protection (TDE / DDM / Always Encrypted), index maintenance, backup/restore drills; `@db-migration-planner` for expand/contract sequencing |
...
pilot-core ships 16 hook scripts covering every hook event type; pilot-sql adds a migration safety verifier:
...
| PreToolUse | `Write\|Edit\|MultiEdit` | `pre-spec-write.js` | Blocks writes to `.claude/specs/` if any of the 8 required spec template sections are absent |
...
| PreToolUse | `Bash` | `pre-scaffold-validate.js` | Blocks `ng generate` / `dotnet new` when the feature slug already exists as a route, controller, or table name |
| PreToolUse | `Bash` | `pre-action-boundary-check.js` | Blocks `kubectl apply`, Azure live-resource mutations, and prod `docker run`; warns on push to `main`/`master` |
...
| PostToolUse | `Bash` | `post-scaffold-build.js` | On `dotnet build` / `ng build` failure, lists newly scaffolded files and suggests a revert path |
| PostToolUse | `Bash` | `post-branch-test.js` | After `git checkout -b` / `git switch -c`, records the branch name and reminds the agent to run the test suite |
